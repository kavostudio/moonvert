...
const selectTriggerVariants = cva(
    ...
    text-popover data-placeholder:text-popover focus-visible:border-ring focus-visible:outline-none focus-visible:ring-[3px] 
  ...
  `,
    {
        variants: {
            size: {
                xs: 'h-7 px-2 text-sm gap-1 rounded-md',
                sm: 'h-8 px-2.5 text-sm gap-1 rounded-md',
...
function SelectTrigger({ className, children, size, ...props }: SelectTriggerProps) {
    return (
        <SelectPrimitive.Trigger className={cn(selectTriggerVariants({ size }), 'no-drag', className)} data-slot="select-trigger" {...props}>
            {children}
            <SelectPrimitive.Icon asChild>
                <ChevronDown className="stroke-popover -me-0.5 h-4 w-4 opacity-100" />
...
function SelectScrollUpButton({ className, ...props }: React.ComponentProps<typeof SelectPrimitive.ScrollUpButton>) {
    return (
        <SelectPrimitive.ScrollUpButton className={cn('flex cursor-default items-center justify-center py-1', 'no-drag', className)} data-slot="select-scroll-up-button" {...props}>
...
function SelectScrollDownButton({ className, ...props }: React.ComponentProps<typeof SelectPrimitive.ScrollDownButton>) {
    return (
        <SelectPrimitive.ScrollDownButton
            className={cn('flex cursor-default items-center justify-center py-1', 'no-drag', className)}
    ...
    return (
        <SelectPrimitive.Portal>
            <SelectPrimitive.Content
                className={cn(
                    'border-border bg-card text-popover data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-xl border shadow-md shadow-black/5',
                    position === 'popper' &&
...
function SelectLabel({ className, ...props }: React.ComponentProps<typeof SelectPrimitive.Label>) {
    return <SelectPrimitive.Label className={cn('no-drag', 'text-popover py-1.5 ps-8 pe-2 text-sm font-medium', className)} data-slot="select-label" {...props} />;
}
...
function SelectItem({ className, children, ...props }: React.ComponentProps<typeof SelectPrimitive.Item>) {
    ...
    return (
        <SelectPrimitive.Item
            className={cn(
                'no-drag',
                'text-popover hover:bg-hover focus:bg-hover relative flex w-full cursor-default items-center rounded-lg py-1.5 text-sm outline-hidden select-none data-disabled:pointer-events-none data-disabled:opacity-50',
                ...
                (indicator && isValidElement(indicator) ? (
                    ...
                    <span className={cn('absolute flex h-3.5 w-3.5 items-center justify-center', indicatorPosition === 'left' ? 'start-2' : 'end-2')}>
                        <SelectPrimitive.ItemIndicator>
                            <Check className="text-popover h-4 w-4" />
                        </SelectPrimitive.ItemIndicator>
                    </span>
                ))}
...
function SelectIndicator({ children, className, ...props }: React.ComponentProps<typeof SelectPrimitive.ItemIndicator>) {
    ...
    return (
        <span
            className={cn('absolute top-1/2 flex -translate-y-1/2 items-center justify-center', indicatorPosition === 'left' ? 'start-2' : 'end-2', 'no-drag', className)}
...
function SelectSeparator({ className, ...props }: React.ComponentProps<typeof SelectPrimitive.Separator>) {
    return <SelectPrimitive.Separator className={cn('bg-border -mx-1.5 my-1.5 h-px', 'no-drag', className)} data-slot="select-separator" {...props} />;
}
