(() => {
    'use strict';
    function injectProxy({ target: e }) {
        if (!window.__REACT_DEVTOOLS_PROXY_INJECTED__) {
            ((window.__REACT_DEVTOOLS_PROXY_INJECTED__ = !0), connectPort(), sayHelloToBackendManager());
            const e = setInterval(() => {
    ...
    function handleMessageFromDevtools(e) {
        window.postMessage({ source: 'react-devtools-content-script', payload: e }, '*');
    }
        ...
        if (o.source === window && o.data)
            switch (o.data.source) {
                ...
                case 'react-devtools-backend-manager': {
                    ...
                    chrome.runtime.sendMessage({ source: e, payload: n });
                    break;
                }
            }
    }
    function handleDisconnect() {
        (window.removeEventListener('message', handleMessageFromPage), (e = null), connectPort());
    }
    function connectPort() {
        ((e = chrome.runtime.connect({ name: 'proxy' })),
