    ...
    (() => {
        ...
        const background_setExtensionIconAndPopup = function (e, t) {
            (chrome.action.setIcon({
                ...
                chrome.action.setPopup({ tabId: t, popup: chrome.runtime.getURL(`popups/${e}.html`) }));
        };
        function isRestrictedBrowserPage(e) {
            if (!e) return !0;
            const t = new URL(e).protocol;
            return 'chrome:' === t || 'about:' === t;
        }
        function checkAndHandleRestrictedPageIfSo(e) {
            e && isRestrictedBrowserPage(e.url) && background_setExtensionIconAndPopup('restricted', e.id);
        }
        (chrome.tabs.query({}, (e) => e.forEach(checkAndHandleRestrictedPageIfSo)),
            chrome.tabs.onCreated.addListener((e) => checkAndHandleRestrictedPageIfSo(e)),
            chrome.tabs.onUpdated.addListener((e, t, n) => {
                t.url && isRestrictedBrowserPage(t.url) && background_setExtensionIconAndPopup('restricted', e);
            }));
        function executeScriptInMainWorld({ target: e, files: t, injectImmediately: n }) {
            return chrome.scripting.executeScript({ target: e, files: t, injectImmediately: n, world: chrome.scripting.ExecutionWorld.MAIN });
        }
        const e = ['compact'];
        const t = {};
        ...
        function connectExtensionAndProxyPorts(e, n, o) {
            ...
            function disconnectListener() {
                (e.onMessage.removeListener(extensionPortMessageListener), n.onMessage.removeListener(proxyPortMessageListener), delete t[o].disconnectPipe);
            }
                        ...
                        !(function (e) {
                            ...
                            switch (t?.type) {
                                case 'fetch-file-with-cache': {
                                    ...
                                    t && n
                                        ...
                                        : chrome.runtime.sendMessage({
                                              ...
                                              payload: { type: 'fetch-file-with-cache-error', url: n, value: null },
                                          });
                                    break;
                                }
                                case 'inject-backend-manager': {
                                    ...
                                    executeScriptInMainWorld({ injectImmediately: !0, target: { tabId: t }, files: ['/build/backendManager.js'] }).then(
                                        () => {},
                                        (e) => {
                                            console.error('Failed to inject backend manager:', e);
                                        },
                                    );
                                    break;
                                }
                        ...
                        !(function (t, n) {
                            ...
                            'require-backends' === o?.type &&
                                o.versions.forEach((t) => {
                                    e.includes(t) &&
                                        executeScriptInMainWorld({ injectImmediately: !0, target: { tabId: n.tab.id }, files: [`/build/react_devtools_backend_${t}.js`] });
                                });
                        })(t, n);
                        break;
